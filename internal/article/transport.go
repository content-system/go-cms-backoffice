...
func NewArticleTransport(db *sql.DB, logError core.Log, writeLog core.WriteLog, action *core.ActionConfig) (ArticleTransport, error) {
	...
	articleSearchBuilder, err := adapter.NewSearchAdapter[Article, string, *ArticleFilter](db, "news", queryArticle)
	if err != nil {
		return nil, err
	}
	articleRepository, err := adapter.NewAdapter[Article, string](db, "news")
	if err != nil {
		return nil, err
	}
