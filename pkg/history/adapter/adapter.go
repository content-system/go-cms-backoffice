...
func NewHistoryAdapter(db *sql.DB, generate func(ctx context.Context) (string, error), buildParam func(int) string, stype string, resource string, table string, user string, time string, opts ...string) *HistoryAdapter {
	var historyId, id, action, data, tx string
	if len(opts) > 0 {
		historyId = opts[0]
	} else {
	...
	return &HistoryAdapter{DB: db, Generate: generate, BuildParam: buildParam, Type: stype, Tx: tx, Table: table, HistoryId: historyId, Resource: resource, Id: id, User: user, Time: time, Action: action, Data: data}
}
func (a *HistoryAdapter) Create(ctx context.Context, id string, userId string, action string, data map[string]interface{}) (int64, error) {
	...
	query := fmt.Sprintf("insert into %s(%s,%s,%s,%s,%s,%s,%s) values (%s,%s,%s,%s,%s,%s,%s)", a.Table,
		a.HistoryId, a.Type, a.Id, a.User, a.Time, a.Action, a.Data,
	...
	res, err := tx.ExecContext(ctx, query, hid, a.Resource, id, userId, now, action, history.Data)
	if err != nil {
		return -1, err
	}
